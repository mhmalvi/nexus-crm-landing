"use client";
import React from "react";
import Images from "../../images";
import Button from "../../Button";
import Image from "next/image";

const SectionNine = () => {
  return (
    <div className="lg:max-h-full min-h-full w-full flex items-center justify-center lg:mt-0 mt-[64px]">
      <div className="lg:grid lg:grid-cols-8 lg:gap-8 flex flex-col justify-center items-center w-full lg:px-[12vw] px-[4vw] lg:max-h-full min-h-full lg:mt-0">
        <div className="col-span-5 flex flex-col justify-center lg:min-h-full">
          <h6 className="text-start text-blue-500 lg:text-[32px] text-xl lg:leading-10">
            Book a free
          </h6>
          <h1 className="text-blue-800 lg:text-[36px] text-xl text-start lg:leading-10 font-semibold mb-8">
            Demo Now !
          </h1>
          <Image src={Images.crm_demo} alt="QLogo" />
        </div>
        <div className="col-span-3 flex flex-col lg:items-end items-center justify-center lg:min-h-full lg:mt-0 mt-8">
          <h6 className="text-blue-500 lg:text-[32px] text-xl lg:w-full w-[90vw] text-start lg:leading-10 ">
            Unlock the Power
          </h6>
          <h1 className="text-blue-800 lg:text-[32px] font-semibold text-xl w-full text-start">
            {" "}
            Free Demonstration
          </h1>

          <p className="text-blue-800 w-full text-base text-start my-4">
            To experience the whole platform for free, book a Practical Demo.
          </p>

          <div className="flex flex-col justify-start w-full">
            <Button variant={1} title="Book a practical demo ➟" />
          </div>
        </div>
      </div>
    </div>
  );
};

export default SectionNine;
